...
def zarr_write_ds(ds):
    ds.to_zarr(store='/home/ltelfer/scratch/upper_sfs/heter.zarr/exp/',
               mode='a',
               group=f'{group}/{n}');
    return None
...
def pf_workflow(rundir,runname,domain,tcoords):
    ...
    sys.stdout.write('\nwriting parflow ......... ')
    start = time.perf_counter()
    zarr_write_ds(pf.expand_dims({'group':[group],'sample':[int(n)]}))
    end = time.perf_counter()
    elapsed = time.strftime("%H:%M:%S", time.gmtime(end - start))
    sys.stdout.write(elapsed)
    return pf
...
def clm_workflow(rundir,runname,domain,tcoords):
    ...
    sys.stdout.write('\nwriting clm ............. ')
    start = time.perf_counter()
    zarr_write_ds(clm.expand_dims({'group':[group],'sample':[int(n)]}))
    end = time.perf_counter()
    elapsed = time.strftime("%H:%M:%S", time.gmtime(end - start))
    sys.stdout.write(elapsed)
    return clm
...
def calc_workflow(pf,domain,tcoords):
    ...
    start = time.perf_counter()
    zarr_write_ds(calcs.expand_dims({'group':[group],'sample':[int(n)]}))
    end = time.perf_counter()
    elapsed = time.strftime("%H:%M:%S", time.gmtime(end - start))
    sys.stdout.write(elapsed)
    return calcs
...
if __name__ == '__main__':
    ...
    group, n = args.runname.split('.')
    rundir = f'../output_files/{group}/{args.runname}/'
    main()